...
public class DamageModifiers implements Listener {
    ...
    public void onEntityDamage(EntityDamageByEntityEvent e) {
        ...
        if (plugin.getConfig().getBoolean("combat.settings.old_weapon_damage")) {
            if (handItem.getType().name().endsWith("_SWORD")) {
                oldSwordDmg(e, player);
            }
        }
        if (plugin.getConfig().getBoolean("combat.settings.old_tool_damage")) {
        ...
        if (plugin.getConfig().getBoolean("combat.settings.disable_sweep_attacks.enabled")) {
            if (e.getCause().equals(EntityDamageEvent.DamageCause.ENTITY_SWEEP_ATTACK)) {
                disableSweep(e, player, handItem);
            }
        }
        if (plugin.getConfig().getBoolean("combat.settings.old_sharpness")) {
