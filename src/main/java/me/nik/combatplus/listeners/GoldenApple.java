    ...
    public void onEatGoldenApple(PlayerItemConsumeEvent e) {
        ...
        if (handItem == Material.GOLDEN_APPLE) {
            ...
            if (cooldown.containsKey(p)) {
                ...
                taskRun(e);
                if (debug(player)) {
                    player.sendMessage(Messenger.prefix(ChatColor.AQUA + "Golden Apple Cooldown:" + ChatColor.GREEN + " Player: " + player.getName() + ChatColor.YELLOW + " Added to cooldown: " + cooldown.containsKey(p)));
                }
            }
        }
    }
